...
pub struct GovernorConfig {
    ...
    /// *normalized* targets (file path / grep scope, with line ranges and
    /// patterns stripped — see `normalize_target`) is `<= novelty_distinct_floor`,
    /// the loop hard-fails with `LowNoveltyStall`. Catches churn — re-probing a
    /// small set of files with surface-varied greps — that raw read counts and
    /// the exact-match detectors miss, while letting wide legitimate exploration
    /// (many distinct files) pass. `0` disables the detector. Default 24.
    pub novelty_window: usize,
    /// Max distinct normalized targets in `novelty_window` still treated as
    /// churn. A full window collapsing to this few distinct files/scopes is
    /// re-probing, not exploration. Default 6.
    pub novelty_distinct_floor: usize,
...
pub struct ModelOverride {
    ...
    pub read_only_stall_threshold: Option<usize>,
...
impl Config {
    ...
    pub fn resolve_for_model(&mut self, model: &str) {
        ...
        if let Some(v) = over.read_only_stall_threshold {
            self.governor.read_only_stall_threshold = v;
        }
        if let Some(v) = over.novelty_window {
            self.governor.novelty_window = v;
        }
        if let Some(v) = over.novelty_distinct_floor {
            self.governor.novelty_distinct_floor = v;
        }
...
"#,
    ...
    #[test]
    fn governor_novelty_action_defaults_to_advisory() {
        assert_eq!(
            GovernorConfig::default().novelty_action,
            NoveltyAction::Advisory
        );
    }

    #[test]
    fn governor_config_without_novelty_action_key_loads_as_advisory() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("config.toml");
        std::fs::write(
            &path,
...
"#,
        ...
        let cfg = Config::load(&path).unwrap();
        assert_eq!(cfg.governor.novelty_action, NoveltyAction::Advisory);
    }

    #[test]
    fn resolve_for_model_applies_novelty_action_terminate_override() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("config.toml");
        std::fs::write(
            &path,
...
"#,
        ...
        let mut cfg = Config::load(&path).unwrap();
        assert_eq!(
            cfg.governor.novelty_action,
            NoveltyAction::Advisory,
            "global default parses as advisory"
        );
        cfg.resolve_for_model("m");
        assert_eq!(
            cfg.governor.novelty_action,
            NoveltyAction::Terminate,
            "per-model override flips the resolved action to terminate"
        );
    }
...
&config_path,
    ...
    #[test]
    fn model_override_pricing_absent_parses() {
        let dir = tempfile::tempdir().unwrap();
        let config_path = dir.path().join("config.toml");
        std::fs::write(
            &config_path,
